<g:form name="identificationForm" id="identificationForm" method="post" role="form" class="form-horizontal" enctype="multipart/form-data"
        url="[controller: 'openAccount', action: 'saveAttachment']" onSubmit="return false;">
            ...
            <table id="identificationTable" class="table table-bordered table-striped table-hover table-condensed">
                ...
                <g:each in="${personalInfo?.attachments}" var="attachments" status="i">
                    ...
                    <tr>
                        ...
                        <td class="actions ">
                            <div class="btn-group">
                                <sec:access controller="openAccount" action="downloadIdentification">
                                    <a class="btn btn-sm identification-download-link" href="${g.createLink(controller: 'openAccount', action: 'downloadIdentification', params: [attachmentId: attachments?.id])}" id="${attachments.id}" personalId="${personalInfo?.id}" title="Download">
                                        <i class="glyphicon glyphicon-download"></i>
                                    </a>
                                </sec:access>
                                <sec:access controller="openAccount" action="editIdentification">
                                    <a class="btn btn-sm identification-edit-link" href="" id="${attachments.id}" personalId="${personalInfo?.id}" title="Edit">
                                        <i class="glyphicon glyphicon-pencil"></i>
                                    </a>
                                </sec:access>
                                <sec:access controller="openAccount" action="deleteIdentification">
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#identificationForm").submit(function (e) {
            ...
            $.ajax({
                url: '${createLink(controller: 'openAccount', action: 'saveAttachment')}',
                type: 'post',
                data: formData,
                dataType: 'json',
                    ...
                    if(data.isError == false){
                        if(data.add==true){
                            identificationTable.row.add( [
                                ...
                                "<div class='btn-group'><a class='btn btn-sm identification-download-link' href='${g.createLink(controller: 'openAccount', action: 'downloadIdentification')}?attachmentId="+data.attachments.id+"' id="+data.attachments.id+" personalId="+data.personalInfo.id+" title='Download'><i class='glyphicon glyphicon-download'></i></a><a class='btn btn-sm identification-edit-link' href='' id="+data.attachments.id+" personalId="+data.personalInfo.id+" title='Edit'><i class='glyphicon glyphicon-pencil'></i></a><a class='btn btn-sm delete btn-danger identification-delete-link' href='' id="+data.attachments.id+"  personalId="+data.personalInfo.id+" title='Delete'><i class='glyphicon glyphicon-remove'></i></a></div>"
                            ] ).draw();
        ...
        $('#identificationTable').on('click', 'a.identification-delete-link', function(e) {
            ...
            jQuery.ajax({
                type: 'POST',
                dataType: 'json',
                url: "${g.createLink(controller: 'openAccount',action: 'deleteIdentification')}?id="+id+"&personalId="+personalId,
...
//                    $('#'+updateDiv).html(data);
        ...
        $('#identificationTable').on('click', 'a.identification-edit-link', function(e) {
            ...
            jQuery.ajax({
                type: 'POST',
                dataType: 'json',
                url: "${g.createLink(controller: 'openAccount',action: 'editIdentification')}?id="+id+"&row="+row,
                success: function (data, textStatus) {
                    if(data.isError == false){
