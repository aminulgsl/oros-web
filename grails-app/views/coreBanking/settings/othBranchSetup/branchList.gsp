...
//                    $('#'+updateDiv).html(data);
        ...
        $('#sample-table-2').on('click', 'a.edit-branch', function (e) {
            ...
            jQuery.ajax({
                type: 'POST',
                url: "${g.createLink(controller: 'othBranchSetup',action: 'update')}?id=" + branchId,
                success: function (data, textStatus) {
                    $('#page-content').html(data);
                },
                error: function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        $('#sample-table-2').on('click', 'a.delete-branch', function (e) {
            ...
            if (confirmDel == true) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: "${g.createLink(controller: 'othBranchSetup',action: 'delete')}?id=" + branchId,
                    success: function (data, textStatus) {
                        $('#page-content').html(data);
                    },
                    error: function (XMLHttpRequest, textStatus, errorThrown) {
//                    $('#'+updateDiv).html(data);
    ...
    function getActionButtons(nRow, aData) {
        var actionButtons = "";
        actionButtons += '<sec:access controller="othBranchSetup" action="update"><span class="col-xs-6"><a href="" branchId="' + aData.DT_RowId + '" class="edit-branch" title="Edit">';
        actionButtons += '<span class="green glyphicon glyphicon-edit"></span>';
        actionButtons += '</a></span></sec:access>';
        actionButtons += '<sec:access controller="othBranchSetup" action="delete"><span class="col-xs-6"><a href="" branchId="' + aData.DT_RowId + '" class="delete-branch" title="Delete">';
