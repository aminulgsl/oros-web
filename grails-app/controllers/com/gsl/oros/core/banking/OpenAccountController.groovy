...
class OpenAccountController {
    ...
    def productInfo() {
        ...
        render(view: 'fees&TermsInfo', model: [accountType:accountType, productId:productId, productName:params.productName,
    ...
    def savePersonalInfo(PersonalInfoCommand personalInfoCommand) {
        if (!request.method == 'POST') {
            ...
            if(personalInfoCommand.id){ // update
                ...
                AccOpenRequest accOpenRequest = new AccOpenRequest(user: loggedUser.id, personalInfo: savedPersonalInfo.id, accountType: params.accountType, productId: params.productId, productName: params.productName, status: RequestStatus.DRAFT, requestDate: new Date())
    ...
    @Secured(['ROLE_SUPER_ADMIN'])
    def saveOtherBankAccount(BankAccountCommand otherBankAccountCommand){
        if (!request.method == 'POST') {
            flash.message = "This action is not allowed!"
            render (view: 'openingForm')
        }
