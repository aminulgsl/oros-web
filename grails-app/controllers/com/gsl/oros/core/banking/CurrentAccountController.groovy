...
class CurrentAccountController {
    ...
    @Secured(['ROLE_SUPER_ADMIN'])
    def saveNominee(CurrentNomineeCommand nomineeCommand){
        if (!request.method == 'POST') {
            flash.message = "This action is not allowed!"
    ...
    @Secured(['ROLE_SUPER_ADMIN'])
    def saveOtherBankAccount(CurrentBankAccountCommand otherBankAccountCommand){
        if (!request.method == 'POST') {
            flash.message = "This action is not allowed!"
            render (view: 'openingForm')
        }
    ...
    @Secured(['ROLE_SUPER_ADMIN'])
    def saveAttachment(CurrentAttachmentsCommand attachmentsCommand){
...
class CurrentBankAccountCommand {
    Long id
    Long personalId
    String bankName
    String ibanPrefix
